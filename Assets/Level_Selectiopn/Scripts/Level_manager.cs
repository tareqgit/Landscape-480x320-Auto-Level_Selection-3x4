...
public class Level_manager : MonoBehaviour
{
    ...
    void Awake()
    {
...

        {
            ...
            GameObject ver_parent = _CreateRect_GameObject("Vertical_Parent_" + page + "", 0, 0, 55, 30, LevelSelection_parent.transform);
        ...
        GameObject m_next = _createButton("next", LevelSelection_parent.transform, -2, 0, -3, 0, 0.6715f, 0, 1, 0.115625f);
        m_next.AddComponent<Button>().onClick.AddListener(_Next);

        GameObject m_prev = _createButton("prev", LevelSelection_parent.transform, 0, -1, -3, 0, 0, 0, .33125f, 0.115625f);
        m_prev.AddComponent<Button>().onClick.AddListener(_Prev);

        GameObject m_back = _createButton("Back", LevelSelection_parent.transform, 0, -2, 0, -1, 0, 0.85f, .2085f, 1f);
        m_back.AddComponent<Button>().onClick.AddListener(_Back);

        GameObject m_Page = _createText("Page_no", LevelSelection_parent.transform, -3.54999f, 1.049999f, -1.40000f, 0, .361f, 0, 0.6457f, 0.1218f);
        ...
        Button NextTime = _createButton("NextTime", Locked_panel.transform, 112.9483f, 117.8f, 70, 71.2f, .7586f, 0, 1, .2187f).AddComponent<Button>();
        NextTime.onClick.AddListener(_NextTime);

        Button BuyNow= _createButton("BuyNow", Locked_panel.transform, 118.5f, 118.1f, 70, 71.2f, 0, 0, 0.245f, 0.2187f).AddComponent<Button>();
